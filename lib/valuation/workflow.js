 ...
 *
 * STAGE 3B — Market Data Collection  (frontend → /listing_stream → listing_search.py)
...
export function buildWorkflowFromEvents(events = []) {
  ...
  const VISIBLE_TYPES = new Set([
    ...
    "comparable_results",
      // Stage 3B — Market Data Collection
    "listing_results",
    "transaction_results",
...
function stageForType(type) {
  switch (type) {
    ...
    // Stage 3B — Market Data Collection (/listing_stream, listing_pipeline)
...
function titleForType(type) {
  const map = {
    ...
    done:                     "Valuation Pipeline Complete",
    error:                    "Valuation Pipeline Error",
  };
  return map[type] || "Workflow Step";
}

function countUniqueDroppedComparables(droppedComparables) {
...
function describeEvent(event) {
  ...
  switch (event.type) {
    ...
    case "comparable_results": {
      const accepted = Array.isArray(c?.comparables) ? c.comparables.length : (c?.total_found || 0);
      const total = accepted;
      const dropped = countUniqueDroppedComparables(c?.dropped_comparables);
      const source = c?.comparable_source;
      const sourceLabel = source === "web" ? "Agent web search"
        : source === "db" ? "internal database"
        : source === "both" ? "Agent web search + transaction database"
        : "web search";
      const droppedSuffix = dropped > 0 ? ` ${dropped} filtered out.` : "";
      return `${total} comparable project${total !== 1 ? "s" : ""} identified via ${sourceLabel}.${droppedSuffix}`;
    }
    ...
    case "area_age_recalc": {
      ...
      const val = approach === "cost"
        ...
        : (c?.new_market_value ? `Property Value: ₹${Number(c.new_market_value).toLocaleString()}` : null);
      return [
    ...
    case "done":
      return typeof c === "string" && c ? c : "All Valuation Pipeline stages completed successfully.";

    case "error":
      return c?.message || c?.error || (typeof c === "string" ? c : "An unexpected Valuation Pipeline error occurred.");
