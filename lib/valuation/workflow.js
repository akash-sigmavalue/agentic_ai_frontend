...
function titleForType(type) {
  const map = {
    ...
    factorial_analysis_result:"LLM Rate Factoring Complete",
...
function describeEvent(event) {
  ...
  switch (event.type) {
    ...
    case "comparable_results": {
      ...
      const sourceLabel = source === "web" ? "LLM web search"
        : source === "db" ? "internal database"
        : source === "both" ? "LLM web search + internal database"
        : "web search";
    ...
    case "factorial_analysis_result": {
      ...
      return parts.length > 0 ? parts.join(" · ") : "LLM factorial analysis and rate derivation complete.";
    }
