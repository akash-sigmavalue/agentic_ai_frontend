...
export default function ConnectorPageClient() {
  ...
  const handleRunWorkflow = async (promptOverride?: string, options: ProcessWorkflowOptions = {}) => {
    ...
    try {
      const result = await processWorkflow(cleaned, options);
  ...
  const handleConvUserReply = (input: string) => {
    ...
    const partialIntent = response?.partial_intent || null;
    if (response?.status === "needs_clarification" && partialIntent) {
      void handleRunWorkflow(trimmed, {
        partial_intent: partialIntent,
        team_context: { partial_intent: partialIntent },
        user_answer: trimmed,
      });
      return;
    }
      ...
      <div className="relative z-10 ml-20 flex h-full min-h-0 flex-col pt-20">
        <section className="grid flex-1 min-h-0 gap-5 px-6 pb-6 pt-6 xl:grid-cols-[0.95fr_1fr_1.08fr]">
          ...
          <OutputSectionConnector
            ...
            convStep={convStep}
            onSendPrompt={(nextPrompt, options) => void handleRunWorkflow(nextPrompt, options)}
          />
        </section>
      </div>
    </main>
