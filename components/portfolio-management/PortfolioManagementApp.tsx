...
const API_ROUTES = {
  ...
  portfolioFlatRefresh: '/portfolio/portfolio-flat/refresh',
  uploadPreview: (mode, sectionKey) => mode === 'global' ? '/portfolio/uploads/global/preview' : `/portfolio/uploads/${sectionKey}/preview`,
  uploadAiPreview: (mode, sectionKey) => mode === 'global' ? '/portfolio/uploads/global/ai-preview' : `/portfolio/uploads/${sectionKey}/ai-preview`,
  uploadSectionAutoImport: (sectionKey) => `/portfolio/uploads/${sectionKey}/auto-import`,
  uploadGlobalMapping: (uploadId) => `/portfolio/uploads/global/${uploadId}/mapping`,
  uploadMappingPayload: (uploadId) => `/portfolio/uploads/${uploadId}/mapping`,
  uploadMapping: (uploadId) => `/portfolio/uploads/${uploadId}/mapping`,
...
export default function PortfolioManagementApp() {
  ...
  const [uploadMessage, setUploadMessage] = useState('');
  const [uploadMode, setUploadMode] = useState('section');
  const [mappingMode, setMappingMode] = useState('normal_ai');
  const [excelWorkbookData, setExcelWorkbookData] = useState({});
  const [globalColumnMappings, setGlobalColumnMappings] = useState({});
  const [backendUploadPreview, setBackendUploadPreview] = useState(null);
  const [portfolioAssets, setPortfolioAssets] = useState([]);
  ...
  const handleExcelFile = async (event) => {
    ...
    try {
      setApiBusy(true);
      setUploadImportSummary(null);
      const isFullAiMapping = mappingMode === 'full_ai';
      setUploadMessage(
        isFullAiMapping
          ? 'Analyzing full workbook with AI. This may take longer for complex Excel files...'
          : 'Uploading file to backend for preview and normal AI mapping...'
      );
      const formData = new FormData();
      formData.append('file', file);
      const previewEndpoint = isFullAiMapping
        ? API_ROUTES.uploadAiPreview(uploadMode, selectedSection.backendKey)
        : API_ROUTES.uploadPreview(uploadMode, selectedSection.backendKey);
      const preview = await apiRequest(
        previewEndpoint,
        { method: 'POST', body: formData }
      ...
      setBackendUploadPreview(preview);
      setLatestUploadTokenUsage(buildTokenUsageSummary(preview, isFullAiMapping ? 'full_ai' : uploadMode));
      const tables = preview.tables || [];
      const firstTable = tables[0] || {};
      const rows = firstTable.preview_rows || [];
      const columns = firstTable.uploaded_columns || Object.keys(rows[0] || {});
      ...
      if (uploadMode === 'global') {
        ...
        setExcelRows(rows);
        setExcelColumns(columns);
        setUploadMessage(`${tables.length} table(s) detected by backend using ${isFullAiMapping ? 'Full AI Analysis Mapping' : 'Normal AI Mapping'}. Review mapping and click Confirm Mapping & Import.`);
        return;
      }
      ...
      setExcelColumns(columns);
      setColumnMapping(autoMapping);
      setUploadMessage(`${rows.length} preview row(s) detected by backend using ${isFullAiMapping ? 'Full AI Analysis Mapping' : 'Normal AI Mapping'}. Review field-column mapping and click Apply Mapping.`);
    } catch (error) {
      console.error(error);
      const modeFailureHint = mappingMode === 'full_ai'
        ? 'Full AI Analysis Mapping failed. Try Normal AI Mapping if your file has a clean table.'
        : 'Normal AI Mapping failed. Try Full AI Analysis Mapping if your file contains charts, dashboards, merged cells, or multiple tables.';
      setUploadMessage(`${modeFailureHint} Error: ${error.message}`);
      setApiStatus(`Excel upload failed: ${error.message}`);
    } finally {
      setApiBusy(false);
      event.target.value = '';
