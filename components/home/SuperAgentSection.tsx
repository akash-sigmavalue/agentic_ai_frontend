...
export default function SuperAgentSection() {
  ...
  return (
    <div id="super-agent-orbit" className="relative overflow-x-hidden bg-[#04091d] text-[#f7f8ff] font-sans box-border [&_*]:box-border">

      {/* Background gradients */}
      <div className="pointer-events-none absolute inset-0 z-0 bg-[radial-gradient(circle_at_50%_43%,rgba(84,61,255,0.22),transparent_27rem),radial-gradient(circle_at_50%_100%,rgba(26,20,110,0.28),transparent_38rem)]" />
      <div
        className="pointer-events-none absolute inset-0 z-0 opacity-20 [background-image:radial-gradient(circle,rgba(92,116,255,0.9)_1px,transparent_1px)] [background-size:32px_32px]"
        style={{ WebkitMaskImage: 'linear-gradient(to bottom, #000 0%, #000 75%, transparent 100%)', maskImage: 'linear-gradient(to bottom, #000 0%, #000 75%, transparent 100%)' }}
      />

      {/* Main Container */}
      <main className="relative z-10 mx-auto w-[min(1600px,100%)] px-[30px] pb-[20px] pt-[28px] max-md:px-[12px] max-md:pb-[16px] max-md:pt-[22px]">
        {/* ── SINGLE IMMERSIVE STAGE — 3D orbit on desktop, stacked layout on mobile ── */}
        <section
        ...
        >
          ...
          >
            {/* ── 3D Tilted Floor Base Plane (rotateX 56deg) ── */}
            <div
            ...
            >
              ...
              <div className="absolute inset-0 rounded-full border border-[rgba(120,100,255,0.35)] shadow-[0_0_140px_rgba(76,54,255,0.22),inset_0_0_120px_rgba(48,37,172,0.18)]">
                <div className="absolute inset-[8%] rounded-full border border-dashed border-[rgba(58,190,255,0.25)]" />
                <div className="absolute inset-[18%] rounded-full border border-[rgba(140,120,255,0.18)]" />
                <div className="absolute inset-[30%] rounded-full border border-dashed border-[rgba(160,130,255,0.12)]" />
              </div>

              {/* 3D Floor Grid Radial Glow */}
              <div className="absolute inset-[15%] rounded-full bg-[radial-gradient(circle_at_50%_50%,rgba(92,70,255,0.22),transparent_70%)] blur-[24px]" />

              {/* Network lines – SVG on the 3D tilted plane */}
              <svg className="absolute inset-0 h-full w-full opacity-40 max-md:opacity-[0.15] pointer-events-none" aria-hidden="true" style={{ overflow: 'visible' }}>
                <defs>
                  <linearGradient id="lineGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                    <stop offset="0%" stopColor="rgba(130, 110, 255, 0.60)" />
            ...
            >
              ...
              <div className="absolute inset-[10%_0%_4%] -z-10 rounded-full bg-[radial-gradient(ellipse,rgba(96,72,255,0.90),transparent_60%)] blur-[44px]" />
              {/* Wide ambient haze */}
              <div className="absolute inset-[-18%] -z-10 rounded-full bg-[radial-gradient(ellipse,rgba(50,30,185,0.45),transparent_55%)] blur-[75px]" />
              {/* 3D Isometric floor glow disc */}
              <div className="absolute bottom-[4%] left-1/2 -z-10 h-[16%] w-[60%] -translate-x-1/2 rounded-full bg-[rgba(90,65,255,0.65)] blur-[30px]" />
              <img
                className="block w-full rounded-[34%_34%_12%_12%] mix-blend-screen saturate-[1.18] drop-shadow-[0_45px_100px_rgba(62,44,255,0.60)]"
                style={{
              ...
              />
              {/* Central Node badge */}
              <div className="absolute bottom-[6%] left-1/2 flex -translate-x-1/2 items-center gap-[7px] whitespace-nowrap rounded-full border border-[rgba(141,122,255,0.50)] bg-[rgba(7,13,42,0.92)] px-[13px] py-[7px] text-[11px] font-[750] text-[#d4d0ff] shadow-[0_4px_20px_rgba(80,55,255,0.3)] max-md:hidden">
                <Sparkles className="h-[12px] w-[12px]" /> Central Node
              </div>
            </div>

            {/* ── 3D Orbiting Mini-Cards (Desktop only: md:flex, hidden on mobile for clean UX) ── */}
            {AGENTS.map((agent, idx) => {
              ...
              // Position on the orbit (x: -46% to +46%, y: compressed in 3D projection)
              const x = 50 + Math.cos(rad) * 46;
              const y = 50 + Math.sin(rad) * 36; // 3D vertical perspective compression
              const AgentIcon = agent.icon;

              // 3D Depth logic
              const sinVal = Math.sin(rad); // -1 (top/back) to +1 (bottom/front)
              const isBehind = sinVal < 0;
              const zDepth = Math.round(sinVal * 120); // -120px to +120px 3D depth
              ...
              return (
                <div
                  key={`mini-${idx}`}
                  className="max-md:hidden absolute flex min-h-[62px] w-[148px] max-lg:w-[132px] cursor-pointer items-start gap-[9px] rounded-[12px] border border-[rgba(126,110,255,0.32)] bg-[linear-gradient(145deg,rgba(14,23,61,0.95),rgba(7,13,40,0.82))] p-[10px_11px] text-[#f7f8ff] shadow-[0_14px_38px_rgba(0,0,0,0.35)] backdrop-blur-[14px] origin-center will-change-transform hover:border-[rgba(160,140,255,0.85)] hover:shadow-[0_18px_48px_rgba(0,0,0,0.45),0_0_26px_rgba(100,75,255,0.35)]"
                  style={{
                    left: `${x}%`,
                    top: `${y}%`,
                ...
                >
                  <div className="flex h-[30px] w-[30px] shrink-0 place-items-center justify-center rounded-[10px] bg-[rgba(108,88,255,0.18)] text-white shadow-[inset_0_0_0_1px_rgba(144,127,255,0.18)]">
                    <AgentIcon className="h-[14px] w-[14px]" />
                  </div>
                  <div className="min-w-0 pt-[1px]">
                    <div className="line-clamp-2 text-[10.5px] font-[800] leading-[1.18] text-[#f4f5ff]">
                      {agent.name}
                    </div>
                    <div className="mt-[5px] inline-flex text-[8px] font-[850] tracking-[0.09em] text-[#00ddb8]">
                      ● ACTIVE
                    </div>
                  </div>
                </div>
          ...
          {/* ── Mobile Agent Quick Scroll Pills (visible max-md:) ── */}
            ...
            {AGENTS.map((agent, idx) => {
              ...
              return (
                <button
                  ...
                  onClick={() => setActiveIndex(idx)}
                  className={`flex shrink-0 items-center gap-2 px-3 py-2 rounded-full border text-xs font-semibold transition-all snap-center ${isActive
          ...
          {/* ── Nav arrows ── */}
          <button
            className="absolute left-[max(8px,1.5vw)] top-[38%] max-md:top-[130px] z-[150] flex h-[52px] w-[52px] max-md:h-[40px] max-md:w-[40px] -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border border-[rgba(139,122,255,0.6)] bg-[rgba(9,16,47,0.82)] text-[#d4d0ff] shadow-[0_0_30px_rgba(80,57,255,0.17)] transition-all duration-200 hover:scale-105 hover:border-[#9e8cff] hover:text-white hover:shadow-[0_0_38px_rgba(80,57,255,0.38)]"
          ...
          <button
            className="absolute right-[max(8px,1.5vw)] top-[38%] max-md:top-[130px] z-[150] flex h-[52px] w-[52px] max-md:h-[40px] max-md:w-[40px] -translate-y-1/2 cursor-pointer items-center justify-center rounded-full border border-[rgba(139,122,255,0.6)] bg-[rgba(9,16,47,0.82)] text-[#d4d0ff] shadow-[0_0_30px_rgba(80,57,255,0.17)] transition-all duration-200 hover:scale-105 hover:border-[#9e8cff] hover:text-white hover:shadow-[0_0_38px_rgba(80,57,255,0.38)]"
          ...
          {/* ════════════════════════════════════════════════════════
              ...
              Floating on Desktop (top-[60%]), Stacked in Flow on Mobile
          ════════════════════════════════════════════════════════ */}
          <div
            key={`card-${activeIndex}`}
            className="absolute top-[78%] left-1/2 z-[100] w-[min(560px,calc(100%-64px))] max-md:relative max-md:top-0 max-md:left-0 max-md:translate-x-0 max-md:w-full rounded-[20px] border border-[rgba(149,132,255,0.58)] bg-[radial-gradient(circle_at_20%_0%,rgba(102,81,255,0.20),transparent_55%),linear-gradient(150deg,rgba(15,24,66,0.96),rgba(5,10,35,0.97))] p-0 text-left shadow-[0_24px_64px_rgba(0,0,0,0.60),0_0_36px_rgba(84,61,255,0.22),inset_0_1px_0_rgba(255,255,255,0.07)] backdrop-blur-[22px]"
          ...
          >
            ...
            {/* Card Header */}
            <div className="flex items-center gap-[13px] border-b border-[rgba(129,115,202,0.15)] px-[20px] py-[15px]">
              <div className="flex h-[44px] w-[44px] shrink-0 items-center justify-center rounded-[11px] bg-[rgba(108,88,255,0.18)] text-[#c8c4ff] shadow-[inset_0_0_0_1px_rgba(144,127,255,0.24)]">
                <ActiveIcon className="h-[20px] w-[20px]" />
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="m-0 text-[17px] max-md:text-[14px] font-bold leading-[1.2] tracking-[-0.01em] text-white">
                  {activeAgent.name}
                </h3>
                <div className="mt-[3px] text-[9.5px] font-[700] uppercase tracking-[0.13em] text-[#836dff]">
                  Sigmavalue AI Agent
                </div>
              </div>
              <div className="rounded-full border border-[rgba(136,118,255,0.25)] bg-[rgba(119,99,255,0.12)] px-[10px] py-[4px] text-[10px] font-bold text-[#d7d3ff] shrink-0">
                {activeIndex + 1} / {total}
              </div>
            </div>

            {/* Description — 2 lines, clean and readable */}
            <div className="px-[20px] pt-[13px]">
              <p className="line-clamp-2 text-[12.5px] max-md:text-[11.5px] leading-[1.68] text-[#9aa0bc]">
                {activeAgent.description}
              </p>
            </div>

            {/* Capability chips — first 5, +N remainder */}
            <div className="flex flex-wrap gap-[6px] px-[20px] pt-[11px] pb-[14px]">
              {activeAgent.capabilities.slice(0, 5).map((c, i) => (
                <span
                  key={i}
                  className="rounded-full border border-[rgba(135,117,255,0.22)] bg-[rgba(116,98,255,0.10)] px-[9px] py-[4px] text-[10px] font-semibold text-[#b8b4f0] transition-colors hover:border-[rgba(135,117,255,0.48)] hover:bg-[rgba(116,98,255,0.20)]"
                >
                  {c}
                </span>
              ))}
              {activeAgent.capabilities.length > 5 && (
                <span className="rounded-full border border-[rgba(135,117,255,0.14)] px-[9px] py-[4px] text-[10px] text-[#505880]">
                  +{activeAgent.capabilities.length - 5} more
                </span>
              )}
            </div>

            {/* Footer — ACTIVE status + Open agent CTA */}
            <div className="flex items-center justify-between gap-[10px] border-t border-[rgba(129,115,202,0.14)] px-[20px] py-[12px]">
              <div className="inline-flex items-center gap-[6px] text-[9px] font-[900] tracking-[0.1em] text-[#08e0bc]">
                <span className="h-[6px] w-[6px] rounded-full bg-[#05deb9] shadow-[0_0_10px_rgba(5,222,185,0.90)]" />
                ACTIVE
              </div>
              <button className="inline-flex items-center gap-[6px] rounded-[10px] bg-gradient-to-br from-[#7660ff] to-[#9278ff] px-[16px] py-[9px] text-[12px] font-bold text-white shadow-[0_6px_20px_rgba(104,82,255,0.38)] transition-all hover:-translate-y-[2px] hover:shadow-[0_12px_28px_rgba(104,82,255,0.52)]">
                Open agent →
              </button>
            </div>
          </div>
