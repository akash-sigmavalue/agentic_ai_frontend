...
import { Network, Map, LayoutDashboard, Sun, Moon, SlidersHorizontal } from 'lucide-react';
    ...
    <header className={`fixed top-0 left-0 z-[1001] flex h-20 w-full items-center justify-between px-10 backdrop-blur-md border-b ${shellClass}`}>
      ...
      <Link href="/dashboard" className="flex items-center gap-4 cursor-pointer">
        <img
          src="/favicon.png"
          alt="Sigmavalue logo"
          className="h-12 w-12 rounded-2xl shadow-lg shadow-indigo-200"
        />
        <div className="flex flex-col">
          <h1 className={`text-xl font-black tracking-tight leading-none mb-1 ${titleClass}`}>Sigmavalue Real Estate OS</h1>
      ...
      <div className="flex items-center gap-8">
        ...
        <div className="hidden lg:flex items-center gap-3">
          ...
          >
            <LayoutDashboard className={`h-4 w-4 transition-colors ${pathname === '/dashboard' ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-600'}`} />
            <span className={`text-[10px] font-black uppercase tracking-widest ${pathname === '/dashboard' ? (isDark ? 'text-indigo-300' : 'text-indigo-700') : pillTextClass}`}>DASHBOARD</span>
          </Link>
          <div 
